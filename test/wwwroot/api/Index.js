(function () {
    ...
    var dom = {
        ...
        choose: document.querySelector(".controls_choose"),
    ...
    /* Теперь нужно настроить взаимодействие с coпользователем. Начнем с запуска воспроизведения. */

    var trackUrls = [

    ];

    function bindIDToTracks() {
        var tracks = document.querySelector("#tracks").children;
    ...
    function url() {
        var url = document.getElementsByClassName("urls");

        for (var i = 0; i < url.length; i++) {
            trackUrls.push(url[i].innerHTML);
        }
    }

    var trackIndex = 0;
    trackIndex.onchange = function () {
    }

    var startPlay = function () {
        url();
        bindIDToTracks();
        document.getElementById("text").innerHTML = document.getElementById(trackIndex).childNodes[3].innerHTML
+" - "            + document.getElementById(trackIndex).childNodes[1].innerHTML;
    ...
    //dom.choose.addEventListener("click", function (event) {

    //    var check = document.getElementsByClassName("controls_choose");
    //   console.log( check.indexOf(event.target));
    ...
    dom.next.addEventListener("click", function () {
        ...
        if (trackIndex < trackUrls.length) {
            document.getElementsByClassName("name").innerHTML = document.getElementById(trackIndex).childNodes[1].innerHTML;
